...
<head>
  ...
  <style>
    ...
    .thumb{margin-top:10px;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,.12);background:#0b1220}
    .thumb img{width:100%;height:180px;object-fit:cover;display:block}
    .thumb iframe{width:100%;height:180px;border:0}
...
<body style="display:block">
  <div class="container">
    <div class="topbar">
      <div>
        <h1>الخبرات / الدورات</h1>
        <div class="muted">كل مرفقاتك بتنحفظ وبتنعرَض هون.</div>
      </div>
      <div class="navrow">
        <a class="btn ghost" href="index.php">الرئيسية</a>
        <button class="btn ghost" type="button" id="toggleUpload">إرفاق خبرة (PDF/صورة)</button>
        <a class="btn primary" href="courses.php">الدورات</a>
      </div>
    </div>
        ...
        <div class="card">
          ...
          <div class="card" id="uploadBox" style="margin-top:12px;display:none">
            <form id="uploadForm" class="row" style="display:flex;gap:10px;flex-wrap:wrap">
              <input name="title" placeholder="عنوان (اختياري) مثل: شهادة Python / خبرة تدريب" style="flex:1;min-width:240px" class="input" />
          ...
          <div class="attachments" id="expAttachments"></div>
          <div class="muted" id="noExp" style="margin-top:10px;display:none">لا يوجد مرفقات بعد.</div>
        </div>
      </div>
    </div>
  </div>
...
async function delAtt(id, type){
  ...
  fd.append("id", id);
  fd.append("type", type);
  const r = await fetch(`${API}/delete.php`, {method:"POST", body:fd, credentials:"include"});
  if(!r.ok){
...
async function load(){
  ...
  const box = document.getElementById("expAttachments");
  box.innerHTML = "";
  if(!arr || arr.length === 0){
    document.getElementById("noExp").style.display = "block";
    return;
  }
  document.getElementById("noExp").style.display = "none";
    ...
    box.innerHTML += `
      <div class="card att attcard">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
          <div>
            <a href="view_attachment.php?type=experience&id=${it.id}" style="text-decoration:none;color:inherit">
              <b>${esc(it.title || it.original_name)}</b>
            </a>
            <div class="muted" style="margin-top:4px">${it.created_at || ""}</div>
          </div>
          <div class="attactions">
            <a class="btn ghost" href="view_attachment.php?type=experience&id=${it.id}">عرض كامل</a>
            <button class="btn danger delbtn" type="button" onclick="delAtt(${it.id}, 'experience')">حذف</button>
          </div>
        </div>

        <a href="view_attachment.php?type=experience&id=${it.id}" style="text-decoration:none">
          <div class="thumb">
